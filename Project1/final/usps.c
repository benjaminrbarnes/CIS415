...
int main() {
    ...
    while ((result = p1getline(0, buf, BUF_SIZE)) != 0) {
        ...
        /* if list of programs has no programs, make process the first & last */
    ...
    while (list_of_programs->num_of_programs > 0) {
         ...
         * this is set to 1 when child throws sigchld and it
         * is confirmed the sigchld is from child completion rather than
         * an interrupt or resume. Look at SIGCHLD handler to see
         * where child_done is changed */
        if (child_done) {
